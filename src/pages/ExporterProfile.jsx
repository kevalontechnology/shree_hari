...
const ExporterProfile = () => {
  ...
  // 2. Multipurpose Filter Logic
  const filteredExporters = exporters.filter((item) => {
    const term = searchTerm.toLowerCase();

    const companyMatch = item.companyName?.toLowerCase().includes(term);
    const consigneeMatch = item.consignee?.toLowerCase().includes(term);
    const iecMatch = item.iecNo?.toLowerCase().includes(term);
    const gstMatch = item.gstNo?.toLowerCase().includes(term);
    const bankMatch = item.bankName?.toLowerCase().includes(term);
    const addressMatch = item.companyAddress?.toLowerCase().includes(term);

    return companyMatch || consigneeMatch || iecMatch || gstMatch || bankMatch || addressMatch;
  });
            ...
            <div className="flex gap-3 items-center">
              ...
              <div className="relative hidden md:block">
                ...
                <input 
                  ...
                  placeholder="Search company, IEC, GST..." 
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  /* 4. filteredExporters map કર્યું */
                  filteredExporters.map((item, idx) => (
                  ...
                  <tr>
                    <td colSpan="5" className="text-center p-6 text-slate-500 font-medium">
                      {searchTerm ? 'No matching exporters found.' : 'No exporters found.'}
                    </td>
                  </tr>
                )}
              </tbody>
