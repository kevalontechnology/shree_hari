...
const BuyerMaster = () => {
  ...
  const handleChange = (e) => {
    ...
    // Check if user selected "ADD_NEW" from currency dropdown
  ...
  const handleOpenEditModal = (item) => {
    ...
    setIsAddingCurrency(false);
    // If the buyer's currency isn't in our default list, append it temporarily
          ...
          <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="text-center p-6 text-slate-500 font-medium">No buyers found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-slate-900/50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-2xl w-[650px] shadow-2xl relative max-h-[90vh] overflow-y-auto border border-slate-200">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex gap-8 mb-6 bg-slate-50 p-4 rounded-xl border border-slate-200 ">
                <label className="flex items-center gap-3 text-sm font-bold text-slate-700  cursor-pointer group">
                  <input 
                    type="checkbox" 
                    name="isShipperAuthorized"
                    checked={formData.isShipperAuthorized} 
                    onChange={handleChange}
                    className="w-5 h-5 rounded border-slate-300  bg-white  text-indigo-600 focus:ring-indigo-500 focus:ring-offset-white"
                  />
                ...
                <label className="flex items-center gap-3 text-sm font-bold text-slate-700 cursor-pointer group">
                  <input 
                    type="checkbox" 
                    name="is24x7Contact"
                    checked={formData.is24x7Contact} 
                    onChange={handleChange}
                    className="w-5 h-5 rounded border-slate-300  bg-white  text-indigo-600 focus:ring-indigo-500 focus:ring-offset-white"
                  />
