...
const ManufacturerMaster = () => {
  ...
  // 2. Search ટેક્સ્ટ ફિલ્ટર લોજિક (Company Name, Address, Permission No, GST No.)
  const filteredManufacturers = manufacturers.filter((item) => {
    const term = searchTerm.toLowerCase();
    const companyMatch = item.companyName ? item.companyName.toLowerCase().includes(term) : false;
    const addressMatch = item.address ? item.address.toLowerCase().includes(term) : false;
    const permissionMatch = item.permissionNumber ? item.permissionNumber.toLowerCase().includes(term) : false;
    const gstMatch = item.gstNo ? item.gstNo.toLowerCase().includes(term) : false;

    return companyMatch || addressMatch || permissionMatch || gstMatch;
  });
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  /* 4. Table render માં filteredManufacturers નો ઉપયોગ કર્યો */
                  ...
                  <tr>
                    <td colSpan="6" className="text-center p-6 text-slate-500 font-medium">
                      {searchTerm ? 'No matching manufacturers found.' : 'No manufacturers found.'}
                    </td>
                  </tr>
                )}
              </tbody>
