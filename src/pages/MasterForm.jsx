...
// // Professional, compact components matching Exporter Profile style
// const InputOutline = ({ label, type = "text", ...props }) => (
//   <div className="flex flex-col gap-1 w-full">
//     <label className="text-[15px] font-bold text-gray-700">{label}</label>
//     <input type={type} className="w-full bg-white text-gray-900 text-[13px] px-3 py-1.5 rounded-sm outline-none border border-gray-300 focus:border-blue-500 transition-colors shadow-sm" {...props} />
//   </div>
// );

// const SelectOutline = ({ label, options, defaultOption, ...props }) => (
//   <div className="flex flex-col gap-1 w-full">
//     <label className="text-[12px] font-bold text-gray-700">{label}</label>
//     <div className="relative">
//       <select className="w-full bg-white text-gray-900 text-[13px] px-3 py-1.5 rounded-sm outline-none border border-gray-300 focus:border-blue-500 appearance-none cursor-pointer transition-colors shadow-sm" {...props}>
//         <option value="">{defaultOption || 'Select...'}</option>
//         {options}
//       </select>
//     </div>
...
// const SectionHeader = ({ title }) => (
//   <div className=" border-b border-b-gray-500  px-4 py-3">
//     <h2 className="text-[#17a2b8] font-bold text-lg uppercase tracking-wide">{title}</h2>
//   </div>
// );
...
//   const [status, setStatus] = useState({ type: '', message: '' });
//   const [formData, setFormData] = useState({
//     invoiceNumber: '', invoiceDate: new Date().toISOString().split('T')[0], currency: 'USD',
//     primaryBuyer: '', manufacturer: '', notifyParties: [''],
//     loadingPort: '', dischargePort: '', gatewayPort: '', 
//     containers: [{ containerNumber: '', lineSealNumber: '', electronicSealNumber: '', type: '', size: '', maxWeightKG: '', punchSeal: 'Cargo' }],
//     products: [{ productType: '', productName: '', quantityUnit: 'Pcs', quantity: 0, pricePerUnit: '', exchangeRate: '', netWeightKG: '', grossWeightKG: '' }],
//     insurance: { percentage: '', amount: '', company: '', policyNumber: '' },
//     exporterDetails: { companyName: '', companyAddress: '', officeAddress: '', officeNumber: '', website: '', consignee: '', iecNo: '', gstNo: '', binNo: '' },
//     buyerDetails: { address: '', nitNumber: '', currency: 'USD', guard: '', shipperAuthorizeName: '', shipperMan24x7: '' },
//     manufacturerDetails: { address: '', permissionNumber: '', gstNo: '' }
...
//       try {
//         const [refRes, expRes] = await Promise.all([api.get('/shipments/reference-data'), api.get('/settings/exporter')]);
//         setReferenceData(refRes.data);
//         if (expRes.data) {
//           setFormData(prev => ({ ...prev, exporterDetails: expRes.data }));
//         }
...
//   // Handlers and helper functions (truncated for brevity, keep your original implementations here)
//   const handleChange = (e) => { /* ... original logic ... */ };
//   const handleNestedChange = (section, field, value) => { /* ... original logic ... */ };
//   const handleArrayChange = (index, field, value, arrayName) => { /* ... original logic ... */ };
//   const addArrayRow = (arrayName, emptyObject) => { /* ... original logic ... */ };
//   const removeArrayRow = (arrayName, index) => { /* ... original logic ... */ };
//   const handleSubmit = async (e) => { /* ... original logic ... */ };


//   return (
      
//       <form className="space-y-6 max-w-[1200px] mx-auto" onSubmit={handleSubmit} >
            
//             {/* EXPORTER DETAILS (Boxy Layout) */}
//             <div className="bg-white rounded-sm shadow-sm border border-gray-300 border-t-[#17a2b8] overflow-hidden">
//               <SectionHeader title="Exporter Details" />
//               <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
//                 <InputOutline label="Company Name" value={formData.exporterDetails.companyName} onChange={(e) => handleNestedChange('exporterDetails', 'companyName', e.target.value)} />
//                 <InputOutline label="Company Address" value={formData.exporterDetails.companyAddress} onChange={(e) => handleNestedChange('exporterDetails', 'companyAddress', e.target.value)} />
//                 <InputOutline label="Office Address" value={formData.exporterDetails.officeAddress} onChange={(e) => handleNestedChange('exporterDetails', 'officeAddress', e.target.value)} />
//                 <InputOutline label="Office Number" value={formData.exporterDetails.officeNumber} onChange={(e) => handleNestedChange('exporterDetails', 'officeNumber', e.target.value)} />
//                 <InputOutline label="Website" value={formData.exporterDetails.website} onChange={(e) => handleNestedChange('exporterDetails', 'website', e.target.value)} />
//                 <InputOutline label="Consignee" value={formData.exporterDetails.consignee} onChange={(e) => handleNestedChange('exporterDetails', 'consignee', e.target.value)} />
//                 <InputOutline label="IEC Number" value={formData.exporterDetails.iecNo} onChange={(e) => handleNestedChange('exporterDetails', 'iecNo', e.target.value)} />
//                 <InputOutline label="GST Number" value={formData.exporterDetails.gstNo} onChange={(e) => handleNestedChange('exporterDetails', 'gstNo', e.target.value)} />
//                 <InputOutline label="BIN Number" value={formData.exporterDetails.binNo} onChange={(e) => handleNestedChange('exporterDetails', 'binNo', e.target.value)} />
//               </div>
//             </div>
...
//                     {formData.primaryBuyer && (
//                       <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-gray-50 border border-gray-200 rounded-sm">
//                         <div className="sm:col-span-2">
//                           <InputOutline label="Address" value={formData.buyerDetails.address} onChange={(e) => handleNestedChange('buyerDetails', 'address', e.target.value)} />
//                         </div>
//                         <InputOutline label="NIT Number" value={formData.buyerDetails.nitNumber} onChange={(e) => handleNestedChange('buyerDetails', 'nitNumber', e.target.value)} />
...
//                       <div key={index} className="flex items-end gap-2">
//                         <div className="flex-1">
//                           <SelectOutline label={index === 0 ? "First Notify" : `Additional Notify ${index + 1}`} value={party} onChange={(e) => handleNotifyChange(index, e.target.value)} defaultOption="Select Notify Party" options={referenceData.buyers.map(b => <option key={b._id} value={b._id}>{b.name}</option>)} />
//                         </div>
//                         {formData.notifyParties.length > 1 && (
//                           <button type="button" onClick={() => removeArrayRow('notifyParties', index)} className="p-2 bg-white text-gray-400 border border-gray-300 rounded-sm hover:text-red-600 hover:border-red-600 transition-colors" title="Remove">
//                             <TrashIcon />
//                           </button>
//                         )}
//                       </div>
...
//                     <button type="button" onClick={() => setFormData({ ...formData, notifyParties: [...formData.notifyParties, ''] })} className="self-start mt-1 px-3 py-1.5 text-[12px] font-bold text-blue-700 bg-blue-50 border border-blue-200 rounded-sm hover:bg-blue-100 transition-colors flex items-center gap-1.5">
//                       + Add Notify Party
//                     </button>
//                   </div>
//                 </div>
...
//                     <InputOutline label="Invoice Number" name="invoiceNumber" value={formData.invoiceNumber} onChange={handleChange} placeholder="INV-001" required />
//                     <InputOutline label="Date" name="invoiceDate" type="date" value={formData.invoiceDate} onChange={handleChange} required />
//                   </div>
//                 </div>
...
//                     <SelectOutline label="Company Name" name="manufacturer" value={formData.manufacturer} onChange={handleChange} defaultOption="Select Manufacturer" options={referenceData.manufacturers.map(m => <option key={m._id} value={m._id}>{m.companyName}</option>)} />
...
//                   label="Select Range / Division / Commissionerate" 
//                   value={formData.rangeDataId} 
//                   onChange={e => setFormData({...formData, rangeDataId: e.target.value})} 
//                   defaultOption="Choose Office Location..." 
//                   options={referenceData?.ranges?.map(r => (
//                     <option key={r._id} value={r._id}>{r.range} - {r.division} - {r.commissionerate}</option>
//                   ))} 
//                 />
//               </div>
...
//               <SectionHeader title="Port Routing" />
//               <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
//                 <SelectOutline label="Loading Port" name="loadingPort" value={formData.loadingPort} onChange={handleChange} defaultOption="Select Port" options={referenceData.ports.filter(p => p.type === 'Loading').map(p => <option key={p._id} value={p._id}>{p.portName}</option>)} />
//                 <SelectOutline label="Discharge Port" name="dischargePort" value={formData.dischargePort} onChange={handleChange} defaultOption="Select Port" options={referenceData.ports.filter(p => p.type === 'Discharge').map(p => <option key={p._id} value={p._id}>{p.portName}</option>)} />
//                 <SelectOutline label="Gateway Port" name="gatewayPort" value={formData.gatewayPort} onChange={handleChange} defaultOption="Select Port" options={referenceData.ports.filter(p => p.type === 'Gateway').map(p => <option key={p._id} value={p._id}>{p.portName}</option>)} />
//               </div>
//             </div>
...
//                   {formData.containers.map((container, index) => (
//                     <div key={index} className="grid grid-cols-[repeat(7,1fr)_auto] gap-3 items-end p-3 border border-gray-200 rounded-sm bg-gray-50">
//                       <InputOutline label="Container No." value={container.containerNumber} onChange={(e) => handleArrayChange(index, 'containerNumber', e.target.value, 'containers')} />
//                       <InputOutline label="Line Seal" value={container.lineSealNumber} onChange={(e) => handleArrayChange(index, 'lineSealNumber', e.target.value, 'containers')} />
//                       <InputOutline label="E-Seal" value={container.electronicSealNumber} onChange={(e) => handleArrayChange(index, 'electronicSealNumber', e.target.value, 'containers')} />
//                       <SelectOutline label="Type" value={container.type} onChange={(e) => handleArrayChange(index, 'type', e.target.value, 'containers')} options={<><option>Dry</option><option>Reefer</option><option>Open Top</option></>} />
//                       <SelectOutline label="Size" value={container.size} onChange={(e) => handleArrayChange(index, 'size', e.target.value, 'containers')} options={<><option>20'</option><option>40'</option><option>40' HC</option></>} />
//                       <InputOutline label="Max Wt (KG)" type="number" value={container.maxWeightKG} onChange={(e) => handleArrayChange(index, 'maxWeightKG', e.target.value, 'containers')} />
//                       <SelectOutline label="Punch Seal" value={container.punchSeal} onChange={(e) => handleArrayChange(index, 'punchSeal', e.target.value, 'containers')} options={<><option>Cargo</option><option>Non-Cargo</option></>} />
                      
//                       {formData.containers.length > 1 && (
//                         <button type="button" onClick={() => removeArrayRow('containers', index)} className="p-2 bg-white text-gray-400 border border-gray-300 rounded-sm hover:text-red-600 hover:border-red-600 transition-colors">
//                           <TrashIcon />
//                         </button>
//                       )}
//                     </div>
//                   ))}
//                   <button type="button" onClick={() => addArrayRow('containers', { containerNumber: '', lineSealNumber: '', electronicSealNumber: '', type: '', size: '', maxWeightKG: '', punchSeal: 'Cargo' })} className="mt-1 px-3 py-1.5 text-[12px] font-bold text-blue-700 bg-blue-50 border border-blue-200 rounded-sm hover:bg-blue-100 transition-colors shadow-sm self-start flex items-center gap-1.5">
//                     + Add Container
//                   </button>
//                 </div>
//               </div>
...
//                   {formData.products.map((product, index) => (
//                     <div key={index} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_auto] gap-3 items-end p-3 border border-gray-200 rounded-sm bg-gray-50">
...
//                         onChange={(e) => handleProductSelect(index, e.target.value)}
//                         defaultOption="Select Product"
//                         options={referenceData?.products?.map(p => (
//                           <option key={p._id} value={p._id}>{p.productName}</option>
//                         ))}
//                       />
...
//                       {formData.products.length > 1 && (
//                         <button type="button" onClick={() => removeArrayRow('products', index)} className="p-2 bg-white text-gray-400 border border-gray-300 rounded-sm hover:text-red-600 hover:border-red-600 transition-colors">
//                           <TrashIcon />
//                         </button>
//                       )}
//                     </div>
//                   ))}
//                   <button type="button" onClick={() => addArrayRow('products', { productId: '', productType: '', productName: '', quantityUnit: 'Pcs', quantity: 0, pricePerUnit: '', exchangeRate: '', netWeightKG: '', grossWeightKG: '' })} className="mt-1 px-3 py-1.5 text-[12px] font-bold text-blue-700 bg-blue-50 border border-blue-200 rounded-sm hover:bg-blue-100 transition-colors shadow-sm self-start flex items-center gap-1.5">
//                     + Add Product
//                   </button>
//                 </div>
//               </div>
...
//             {/* INSURANCE */}
//             <div className="bg-white rounded-sm shadow-sm border border-gray-300 border-t-[#17a2b8] overflow-hidden mb-2">
//               <SectionHeader title="Insurance Details" />
...
//             {/* ACTION BUTTONS */}
//             <div className="flex justify-end gap-3 pt-4 pb-10">
//               <button type="button" className="px-6 py-2.5 text-[13px] font-bold text-gray-700 bg-white border border-gray-300 rounded-sm hover:bg-gray-50 transition-colors shadow-sm">
//                 Save Draft
//               </button>
//               <button onClick={handleSubmit} className="px-6 py-2.5 text-[13px] font-bold text-white bg-[#2563EB] rounded-sm hover:bg-blue-700 transition-colors shadow-sm flex items-center gap-2">
//                 Submit Final Form
//               </button>
//             </div>
//        </form>
