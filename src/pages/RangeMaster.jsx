...
const RangeMaster = () => {
  ...
  // 2. Search ટેક્સ્ટ ફિલ્ટર લોજિક (Range, Codes, Division, Commissionerate બધું જ કવર થાય છે)
  const filteredRanges = ranges.filter((item) => {
    const term = searchTerm.toLowerCase();

    const rangeMatch = item.range ? item.range.toLowerCase().includes(term) : false;
    const rangeCodeMatch = item.rangeCode ? item.rangeCode.toLowerCase().includes(term) : false;
    const divisionMatch = item.division ? item.division.toLowerCase().includes(term) : false;
    const divisionCodeMatch = item.divisionCode ? item.divisionCode.toLowerCase().includes(term) : false;
    const commissionerateMatch = item.commissionerate ? item.commissionerate.toLowerCase().includes(term) : false;
    const commissionerateCodeMatch = item.commissionerateCode ? item.commissionerateCode.toLowerCase().includes(term) : false;

    return rangeMatch || rangeCodeMatch || divisionMatch || divisionCodeMatch || commissionerateMatch || commissionerateCodeMatch;
  });
            ...
            <div className="flex gap-3 items-center">
              ...
              <div className="relative hidden md:block">
                ...
                <input 
                  ...
                  placeholder="Search range, division..." 
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  /* 4. Table render માં filteredRanges નો ઉપયોગ કર્યો */
                  filteredRanges.map((item, idx) => (
                  ...
                  <tr>
                    <td colSpan="5" className="text-center p-6 text-slate-500 font-medium">
                      {searchTerm ? 'No matching range records found.' : 'No range records found.'}
                    </td>
                  </tr>
                )}
              </tbody>
